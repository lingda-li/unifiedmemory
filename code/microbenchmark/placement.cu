...
#define ITER 1
#define OUT_ITER 1
#define SIZE (1024 * 1024 * 9 * 7 * 5 * 30L)
#define STEP (16)
//#define STEP (1)
//#define DELAY_ITER 100000

#if defined(HYB_ALLOC) || defined(HYH_ALLOC)
...
#elif defined(HST_ALLOC)
  cudaMallocHost(&d_input, SIZE*sizeof(int));
#elif defined(HYB_ALLOC) || defined(HYH_ALLOC)
  if (SIZE <= MEM_SIZE) {
...
#else
  cudaMallocHost(&d_input_2, (SIZE-MEM_SIZE)*sizeof(int));
#endif
#else
...
#elif defined(UVM_ALLOC) || defined(HST_ALLOC)
  for (unsigned long long i = 0; i < SIZE; i++) {
    d_input[i] = i;
  }
  timer time_in;
