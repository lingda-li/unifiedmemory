...
struct FuncArgAccessInfoPass : public ModulePass {
  static char ID;
  FuncArgAccessInfoPass() : ModulePass(ID) {}
  MemInfo<FuncArgEntry> FAI; // store analysis result

  virtual bool runOnModule(Module &M) {
    errs() << "  ---- Function Argument Access Frequency Analysis ----\n";
    for (Function &F : M) {
      if (F.isDeclaration())
        continue;
...
struct MemAccessInfoPass : public ModulePass {
  ...
  void calculateAccessFreq(Module &M) {
    ...
    for (Function &F : M) {
      ...
      //errs() << "On function " << F.getName() << "\n";
      BlockFrequencyInfo *BFI = &getAnalysis<BlockFrequencyInfoWrapperPass>(F).getBFI();
      MemInfo<FuncArgEntry> *FAI = &getAnalysis<FuncArgAccessInfoPass>().getFAI();
      for (auto &BB : F) {
        double Freq = (double)BFI->getBlockFreq(&BB).getFrequency() / (double)BFI->getEntryFreq();
        //errs() << Freq << "\n";
        for (auto &I : BB) {
  ...
  void getAnalysisUsage(AnalysisUsage &AU) const override {
    AU.addRequired<BlockFrequencyInfoWrapperPass>();
    AU.addRequired<FuncArgAccessInfoPass>();
  }
};

} // end of anonymous namespace

char FuncArgAccessInfoPass::ID = 0;
